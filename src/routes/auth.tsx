...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Acesso restrito ao painel de gestão da BR Car Seminovos." },
...
function AuthPage() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data }) => {
      if (data.session) void routeSignedInUser(data.session.user.id);
    });

    const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_IN" && session) {
        setTimeout(() => void routeSignedInUser(session.user.id), 0);
      }
    });
      ...
      <div className="surface-ink hidden flex-col justify-between p-12 lg:flex">
        ...
        <div>
          ...
          <p className="mt-4 max-w-md text-ink-muted">
            Estoque, custos, margem por veículo, leads do catálogo e vendas — tudo em um só painel.
          </p>
        </div>
        <p className="text-xs text-ink-muted">Acesso restrito à equipe BR Car.</p>
      </div>
